...
class RLSSMConfig(BaseModelConfig):
    ...
    def get_defaults(
        ...
        -------
        tuple
            ``(None, bounds)`` — the default prior is intentionally ``None`` so that
            ``prior_settings="safe"`` in :class:`~hssm.base.HSSMBase` assigns
            priors from bounds rather than fixing parameters to scalar constants.
            ``params_default`` stores initialisation values, not prior distributions.
        """
