...
def _is_fixed_vector(spec: Any) -> bool:
    """Whether `spec` is a fixed per-regime vector (list/tuple or 1-D ndarray).

    A scalar (shared) and a 0-D ndarray are *not* per-regime — the single
    predicate keeps the `p_outlier` check, the length-K check, and the
    degeneracy check agreeing on what counts as per-regime.
    """
    return isinstance(spec, (list, tuple)) or (
...
class RSSSMConfig(BaseModelConfig):
    ...
    def validate(self) -> None:
        ...
        if "p_outlier" in self.list_params:
            ...
            per_regime = "p_outlier" in self.switching_params or _is_fixed_vector(spec)
            if not per_regime:
                raise NotImplementedError(
        ...
        for name, spec in self.param_specs.items():
            ...
            if _is_fixed_vector(spec) and len(spec) != self.K:
                raise ValueError(
