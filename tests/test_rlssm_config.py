...
def test_rlssm_config_data_property_getter():
    """Test data property getter returns response value."""
    config = RLSSMConfig(
        model_name="test_model",
        list_params=["alpha"],
        decision_process="LAN",
    ...
    # data property should return the same value as response
    assert config.data == ["rt", "response"]
    assert config.data == config.response


def test_rlssm_config_data_property_setter():
    """Test data property setter modifies response value."""
    config = RLSSMConfig(
        model_name="test_model",
        list_params=["alpha"],
        decision_process="LAN",
    ...
    # Setting data should modify response
    config.data = ["reaction_time", "choice"]
    assert config.response == ["reaction_time", "choice"]
    assert config.data == ["reaction_time", "choice"]


def test_rlssm_config_from_rlssm_dict_with_data_field():
    """Test creating RLSSMConfig from dictionary using 'data' field."""
    config_dict = {
        "name": "test_rlssm",
        "description": "Test RLSSM with data field",
        "n_params": 2,
        "list_params": ["alpha", "beta"],
        "decision_model": "LAN",
        "data": ["reaction_time", "choice"],  # Using 'data' instead of 'response'
        "choices": [0, 1],
    }

    config = RLSSMConfig.from_rlssm_dict("test_rlssm", config_dict)

    # response should be set from 'data' field
    assert config.response == ["reaction_time", "choice"]
    assert config.data == ["reaction_time", "choice"]


def test_rlssm_config_from_rlssm_dict_data_precedence():
    """Test that 'data' takes precedence over 'response' in from_rlssm_dict."""
    config_dict = {
        "name": "test_rlssm",
        "list_params": ["alpha"],
        "decision_model": "LAN",
        "response": ["rt", "response"],
        "data": ["reaction_time", "choice"],  # 'data' should take precedence
        "choices": [0, 1],
    }
...
def test_rlssm_config_data_alias_consistency():
    """Test that data and response stay in sync."""
    config = RLSSMConfig(
        model_name="test_model",
        list_params=["alpha"],
        decision_process="LAN",
    ...
    # Modify via response
    config.response = ["new_rt", "new_response"]
    assert config.data == ["new_rt", "new_response"]

    # Modify via data
    config.data = ["final_rt", "final_response"]
    assert config.response == ["final_rt", "final_response"]
