...
def test_interpret_onnx(onnx_session, fixture_path):
    ...
    result_aesara = aes_interpret_onnx(model.graph, data)[0].eval()

    np.testing.assert_almost_equal(result_jax, result_onnx, decimal=4)
    # For some reason aesara and onnx (jax) version results are slightly different
    np.testing.assert_almost_equal(result_aesara, result_onnx, decimal=4)
    ...
    aesara_logp = LAN.make_aesara_logp(model)
    ...
    np.testing.assert_almost_equal(
        aesara_logp(rts_choices, *list_params).eval(), result_onnx_rt, 4
    )
