...
# dependencies = [
#     "graphviz==0.21",
#     "hssm @ git+https://github.com/lnccbrown/HSSM.git@5e8610b9e64e499bcec451db4ab2dc67b47d901d",
#     "marimo==0.24.0",
# ]
# ///
...
No sampling is needed. The inline environment pins the affected HSSM ``main``
revision for a reproducible standalone or Molab run::
...
def _(bmb, hssm, mo):
    ...
    **Bambi {bmb.__version__}** from the active environment.
    ...
    The hosted environment is pinned to HSSM `main` at `5e8610b9` so the
    reported defect remains reproducible even after HSSM is fixed. When opened
    from an HSSM checkout with `--no-sandbox`, the notebook instead diagnoses
    that checkout.
