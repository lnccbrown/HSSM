...
2. **LAN gradient Op is now JAX-convertible.** The `LANLogpVJPOp` emitted when PyTensor differentiates a LAN likelihood symbolically (e.g. ADVI via `model.vi(..., backend="jax")`) had no `jax_funcify` registration, so compiling the gradient graph with the JAX linker raised `NotImplementedError` (#1056). The VJP Op now has a JAX conversion, with gradients verified identical to the default (`perform`) execution path across all input configurations. Note: end-to-end `vi(backend="jax")` additionally requires two upstream PyMC fixes (VI approximation parameters lack static shapes; JAX-backend fits leave `jax.Array`s in shared-variable storage) — until those land, use `backend="c"` for VI (~1.4x slower per ADVI iteration on CPU).
3. **Correct gradients for scalars-only choice-probability (CPN/OPN) likelihoods.** The scalars-only params-only gradient path used a plain `jax.grad` instead of a true VJP, silently dropping the upstream cotangent (chain-rule factor). Any graph applying a nonlinear or scaled transform downstream of such a likelihood — e.g. a `p_outlier` mixture on a choice-only model with all-scalar parameters — fit with incorrect gradients. The path now uses a cotangent-correct VJP, and a regression test asserts the chain rule holds on every backend.
4. **`LANLogpOp`/`LANLogpVJPOp` are now module-level classes** (previously re-defined per `make_jax_logp_ops` call), registered with PyTensor's JAX dispatcher once at import. This stops every model construction from permanently growing PyTensor's dispatch registry with entries that pinned the model's jitted likelihood (including ONNX network weights) for the process lifetime. `LANLogpOp` was also migrated from the deprecated `grad` hook to PyTensor 3's `pullback` API, removing the `FutureWarning` emitted on every symbolic differentiation.
